...
class CAMBdata(F2003Class):
    ...
    def calc_transfers(self, params, this, etat, only_transfers=True, only_time_sources=False):
        """
        ...
        :param this: :class:`TThermoData` instance with relevant data
        :param etat: :class:`TRecfast` instance with relevant data
        ...
        """
        # Ensure params, this, and etat are instances of the correct types
        if not isinstance(params, CAMBparams):
            raise TypeError("params should be an instance of CAMBparams")
        if not isinstance(this, TThermoData):
            raise TypeError("this should be an instance of TThermoData")
        if not isinstance(etat, TRecfast):
            raise TypeError("etat should be an instance of TRecfast")

        # Check parameters (assuming _check_params and _check_powers are defined)
        self._check_params(params)
        if not (only_transfers or only_time_sources):
            self._check_powers(params)

        # Call the Fortran function
        result = camblib.CAMBdata_gettransfers(
            byref(self),
            byref(params),
            byref(this),
            byref(etat),
            c_bool(only_transfers),
            c_bool(only_time_sources)
        )
    ...
    def calc_power_spectra(self, this, etat, params=None):
        """
        ...
        if params is not None:
            self.calc_transfers(params, this, etat, only_transfers=False)
        else:
            self._check_powers()
    ...
    def get_time_evolution(self, q, eta, vars=model.evolve_names, lAccuracyBoost=4, frame='CDM'):
        ...
        try:
            ...
            if ncustom:
                ...
                custom_source_func = ctypes.cast(funcPtr, c_void_p)
            else:
                custom_source_func = c_void_p(0)
